...
export async function reconcileCrm(limit = 25) {
  ...
  for (const { reservation, sync } of candidates) {
    ...
    try {
      ...
      const ghlOpportunityId = synced.id ?? sync.ghlOpportunityId
      await db.update(crmSyncState).set({ lastSuccessfulSync: new Date(), lastSyncStatus: 'synchronized', retryCount: 0, lastError: null, ghlOpportunityId, updatedAt: new Date() }).where(eq(crmSyncState.externalReservationId, reservation.externalReservationId))
      if (ghlOpportunityId) await db.update(reservations).set({ ghlOpportunityId, updatedAt: new Date() }).where(eq(reservations.externalReservationId, reservation.externalReservationId))
